...
class ManipulatorNetwork(SubNet):
    ...
    @actor
    def home(self, gripper_angle=145):
        """
        arm joints to home position
        arg:
            gripper_anger: degree
        """
        return self.run_actor('move_joint', 0.0, -radians(45), 0.0, -radians(135), 0.0, radians(gripper_angle), radians(45))

    # @actor
    # def back_form(self):
    #     return self.run_actor('move_joint', -radians(157), -radians(45), -radians(23), -radians(135), 0.0, radians(145), radians(45))

    @actor
    def place_product(self):
        return self.run_actor('move_joint', 0.0, radians(54), 0.0, -radians(75), 0.0, radians(132), radians(45))
...
#        return arm.wait_until_executed() # Never use this
    ...
    # = Pose Goal =
    @actor
    def move_position(self, position=[0.0, 0.0, 0.1034], orientation=[0.0, 0.0, 0.0, 1.0]):
    ...
    @actor
    def euler_to_quat(self, roll=0.0, pitch=0.0, yaw=0.0):
        r = R.from_euler('xyz', [roll, pitch, yaw], degrees=True)
        q = r.as_quat()  # [x, y, z, w]
        return [q[0], q[1], q[2], q[3]]

    @actor
    def move_to_pose(self, position=[0.2, 0.0, 1.5], quat_xyzw=[0.0, 0.7071067811865475, 0.0, 0.7071067811865476]):
        """
        position: [x, y, z]
        x : 前後 (+前 -後)
        y : 左右（+左 -右）
        z : 上下（+上 -下）(base_linkからの相対位置)
        quat_xyzw: [x, y, z, w]
        (qx, qy, qz) = 回転軸を表すベクトル
        qw = 回転の強さを表す係数
        ...
        例）真上を向く: [0, 0, 0, 1]
            真下を向く: [1, 0, 0, 0]
            真上を向く+gripperを90度回転: [0, 0, 0.707, 0.707]  
            真上を向く+gripperを-90度回転: [0, 0, -0.707, 0.707] 
        """
    ...
    # == Scene Object ACTORs ==
    @actor
    def add_scene_object(self, position, quat_xyzw=[0.0, 0.0, 0.0, 1.0]):
        arm = self.get_value('arm')
    ...
    @actor
    def remove_scene_object(self, object_id="box"):
        arm = self.get_value('arm')
        
        arm.remove_collision_object(
            id=object_id
        )
        return True
